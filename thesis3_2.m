...
% group-1卖家，group-2买家
% 
% multi-homing and single-homing: Group-1分成两段定价，Group-2统一定价
...
% Pb_12 = sol3.Pb_12;
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 4
% the subcase that Pb_11 = Pb_12 (APD) through a different way
% Pb_11 = Pb_1;
% Pb_12 = Pb_1;
% na_1 = simplify(subs(na_1));
% na_2 = simplify(subs(na_2));
...
% nb_2 = simplify(subs(nb_2));
% pi_B = simplify(subs(pi_B));
% eq8 = simplify(subs(diff(pi_B, Pb_1)));
% eq9 = simplify(subs(diff(pi_B, Pb_2)));
% sol3 = solve(eq8, eq9, Pb_1, Pb_2);
% Pb_1 = sol3.Pb_1;
% Pb_2 = sol3.Pb_2;
% pi_A = simplify(subs(pi_A));
% eq5 = simplify(subs(diff(pi_A, Pa_11)));
% eq6 = simplify(subs(diff(pi_A, Pa_12)));
% eq7 = simplify(subs(diff(pi_A, Pa_2)));
% sol4 = solve(eq5, eq6, eq7, Pa_11, Pa_12, Pa_2);
% Pa_11 = sol4.Pa_11;
% Pa_12 = sol4.Pa_12;
% Pa_2 = sol4.Pa_2;
...
% eq5 = simplify(subs(diff(pi_A, Pa_1)));
% eq7 = simplify(subs(diff(pi_B, Pb_1)));
% sol2 = solve(eq5, Pa_1, 'ReturnConditions', true);
% Pa_1 = sol2.Pa_1;
...
syms alpha1 alpha2 V t1 t2 c1 c2 real
syms Pa_1 Pa_2 Pb_1 Pb_2 real
syms na_1 na_2 nb_1 nb_2 la_1 la_2 lb_1 lb_2 real
syms CS1 CS2 CS SW t real
t1 = t;
t2 = t;
t = 1;
...
nb_1 = sol1.nb_1;
nb_2 = sol1.nb_2;
la_1 = (Pa_1 - V + alpha1*(Pa_2 - V))/(alpha1*alpha2 - 1);
la_2 = (Pa_2 - V + alpha2*(Pa_1 - V))/(alpha1*alpha2 - 1);
lb_1 = (Pb_1 - V + alpha1*(Pb_2 - V))/(alpha1*alpha2 - 1);
lb_2 = (Pb_2 - V + alpha2*(Pb_1 - V))/(alpha1*alpha2 - 1);
pi_A = simplify(subs(Pa_1*na_1*(la_1 + lb_1) + Pa_2*na_2*(la_2 + lb_2) - c1*na_1 - c2*na_2));
pi_B = simplify(subs(Pb_1*nb_1*(la_1 + lb_1) + Pb_2*nb_2*(la_2 + lb_2) - c1*nb_1 - c2*nb_2));
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 1
% normal subcase
eq5 = simplify(subs(diff(pi_A, Pa_1)));
eq6 = simplify(subs(diff(pi_A, Pa_2)));
...
% 
% no competition
% syms V_1 V_2 V t1 t2 alpha1 alpha2 real
% syms Pa_1 Pa_2 real
% syms na_1 na_2 real
% syms CS1 CS2 CS SW t real
% t1 = 1;
...
% V_1 = V;
% V_2 = V;
% eq1 = V_1 + alpha1*na_2 - Pa_1 - t1*na_1;
% eq2 = V_2 + alpha2*na_1 - Pa_2 - t2*na_2;
...
% eq5 = simplify(subs(diff(pi_A, Pa_1)));
% eq6 = simplify(subs(diff(pi_A, Pa_2)));
% sol2 = solve(eq5, eq6, Pa_1, Pa_2);
% Pa_1 = sol2.Pa_1;
% Pa_2 = sol2.Pa_2;
% na_1 = simplify(subs(na_1));
% na_2 = simplify(subs(na_2));
% pi_A = simplify(subs(pi_A));
% CS1 = V_1 + na_2*alpha1*na_1 - Pa_1*na_1 - t1*int(t, t, 0, na_1);
% CS2 = V_2 + na_1*alpha2*na_2 - Pa_2*na_2 - t2*int(t, t, 0, na_2);
% SW = CS1 + CS2 + pi_A;
% CS1 = simplify(subs(CS1));
% CS2 = simplify(subs(CS2));
% CS = simplify(subs(CS1 + CS2));
